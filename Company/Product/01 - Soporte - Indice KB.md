# Soporte - Índice KB

Base de conocimiento de soporte para Arche.

## Documentación oficial (centro de ayuda)

- [[Company/Product/docs/index|Inicio]]
- [[Company/Product/docs/faqs|FAQs]]
- [[Company/Product/docs/free-vs-pro|Free vs Pro]]
- [[Company/Product/docs/manage-subscription|Gestionar suscripción]]
- [[Company/Product/docs/delete-account|Eliminar cuenta]]
- [[Company/Product/docs/contact|Contacto]]

## Problemas frecuentes

- (Añadir entradas de KB conforme se creen)

## Procedimientos

- (Añadir procedimientos conforme se documenten)

## Plantilla

- [[Templates/Soporte - KB Entry]]

## Proceso

1. Cuando se resuelve un ticket con una solución nueva, crear entrada KB.
2. Enlazarla desde este índice.
3. Usar el agente `knowledge-curator` para normalizar y mantener.
